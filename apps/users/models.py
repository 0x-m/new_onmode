...
class User(AbstractUser):
    """Abstract model for site users (both customers and sellers)
    ...
    MAX_STORAGE_SIZE = config(
        "MAX_STORAGE", default=20, cast=int
    ...
    phone_num = models.CharField(
        max_length=11, validators=[RegexValidator("^09[0-9]{9}$")], unique=True
    )
    gender = models.CharField(
        max_length=10, choices=[("man", "man"), ("woman", "woman")], blank=True
    )
    user_code = models.CharField(
        max_length=20, default=generate_usecode, editable=False
    )
    ...
    use_custom_storage_capcity = models.BooleanField(
        default=False,
        help_text="check if you want arbitrary storage capacity for this user",
    )
    custom_storage_capacity = models.FloatField(
        default=20, help_text="custom storage in mega bytes"
    )  # in MB
    consumed_storage = models.FloatField(
...
class DepositTransaction(models.Model):
    wallet = models.ForeignKey(
        to=Wallet, related_name="deposits", on_delete=models.CASCADE
    )
    amount = models.PositiveBigIntegerField(default=0)
...
class TicketReply(models.Model):
    ticket = models.ForeignKey(
        to=Ticket, related_name="replies", on_delete=models.CASCADE
    )
    body = models.TextField(max_length=5000)
    user = models.ForeignKey(
        to=User,
