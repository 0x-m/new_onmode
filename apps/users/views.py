...
def verify_code(request: HttpRequest):
    ...
    # get request
    return render(request, "user/verify.html", {"form": VerificationCodeForm()})
    ...
    if request.method == "POST":
        ...
        if amount.startswith("gift_"):
            ...
            params = {
                ...
                + reverse("users:deposit-verify", kwargs={"amount": amount}),
                "currency": "IRT",
            ...
            if res["data"]["code"] == 100:
                ...
                return render(
                    request, "shop/checkout.html", {"deposit_status": "error"}
                )
...
def comments(request: HttpRequest):
    ...
    return render(
        request, "user/dashboard/comments.html", {"page": page, "state": state}
    )
...
def messages(request: HttpRequest):
    ...
    return render(
        ...
        {"messages": messages, "state": "read" if state_bool else "unread"},
    )
...
def ticket(request: HttpResponse, ticket_id):
    ...
    return render(request, "user/dashboard/ticket.html", {"ticket": ticket})
