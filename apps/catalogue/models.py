...
class Shop(models.Model):
    ...
    def inc_product_count(self):
        if not self.has_capacity():
            return Exception(
                "max allowd product restriction"
            )  # NOTE: raising Exception..??
...
class Discount(models.Model):
    ...
    def is_valid(self):
        nw = timezone.now()
        return self.sales < self.max_sales_allowed and (
            self.start_date < nw < self.end_date
        )
...
class PublishedProductManger(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(deleted=False, published=True)
...
def get_brand(request: HttpRequest):
    ...
    products = Product.objects.filter(
        options__option__name="brand", options__value__in=brand_ids
    ).all()
...
class ProductStats(models.Model):
    product = models.OneToOneField(
        to=Product, on_delete=models.CASCADE, related_name="stats"
    )
...
class Collection(models.Model):
    products = models.ManyToManyField(
        to=Product, related_name="collections", blank=True
    )
...
class Favourite(models.Model):
    product = models.ForeignKey(
        to=Product, related_name="likes", on_delete=models.CASCADE
    )
    user = models.ForeignKey(
        to=User, related_name="favourites", on_delete=models.CASCADE
    )
