...
class AddOrderItemTest(TestCase):
    ...
    def test_add_two_products_to_order(self):
        self.client.force_login(self.user)
        res_1 = self.client.post('/cart/add', {
            'product': 1,
            'quantity': 2
        })
        res_2 = self.client.post('/cart/add', {
            'product': 2,
            'quantity': 1
        })

        self.assertJSONEqual(res_1.content, {
            'status': 'added',
            'final_price': 3000
        })

        self.assertJSONEqual(res_2.content, {
            'status': 'added',
            'final_price': 5500
        })
    ...
    def test_add_discounted_product_to_order(self):
        ...
        discount = Discount(percent=10, start_date=nw, end_date=nw +
                            timezone.timedelta(days=1), max_amount=20000)
        discount.save()
        self.product_1.discount = discount
        self.product_1.save()
        discount_2 = Discount(percent=50, start_date=nw, end_date=nw +
                              timezone.timedelta(days=1), max_amount=20000)
        discount_2.save()
    ...
    def test_add_coupon_to_order(self):
        ...
        coupon = Coupon(type=Coupon.TYPE.PERCENT, percent=10, start_date=nw,
                        end_date=nw + timezone.timedelta(days=1), max_amount=10000)
        coupon.save()
        ...
        res_3 = self.client.post('/cart/coupon/add', {
            'code': coupon.code,
            'order_id': 1
        })
        self.assertJSONEqual(res.content, {
        ...
        print(res_3.content)
        self.assertJSONEqual(res_3.content, {
            'status': 'set',
            'final_price': 3555
        })
    ...
    def test_delete_coupon_of_order(self):
        ...
        res_4 = self.client.post('/cart/coupon/delete', {
            'order_id': 1
        })
        self.assertJSONEqual(res.content, {
            'status': 'added',
        ...
        self.assertJSONEqual(res_4.content, {
            'status': 'deleted',
            'final_price': 3950
        })
    ...
    def test_delete_coupon_and_discount(self):
        ...
        re = self.client.get('/cart/refresh', {
            'id': 1
        })
